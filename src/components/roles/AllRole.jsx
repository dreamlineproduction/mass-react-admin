...
const AllRole = () => {
    ...
    // Fetch Data
    let finalUrl = `${API_URL}/roles?page=${pageNumber}&perPage=${perPage}`;
    ...
    const actionDelete = async (id) => {
        ...
        try {
            ...
            response = await response.json();
            if (response.status) {
                const filteredData = roles.filter(item => item.id !== id);
                                                ...
                                                <td>
                                                    <div className="dropdown">
                                                        ...
                                                        <ul className="dropdown-menu">
                                                            <li>
                                                                <Link className="dropdown-item" to={`/roles/edit-role/${item.id}`}>
                                                                    Edit
                                                                </Link>
                                                            </li>   
                                                            {![1,5,6,7].includes(item.id) &&                                                       
                                                            <li>
                                                                <button type="button" className="dropdown-item" onClick={() => deleteRole(item.id)}>
                                                                    Delete
                                                                </button>
                                                            </li>
                                                            }
